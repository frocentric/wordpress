...
trait EncryptionTrait
{
     ...
     * @internal
     */
    protected function encrypt(\NF_FU_VENDOR\GuzzleHttp\Psr7\Stream $plaintext, array $cipherOptions, \NF_FU_VENDOR\Aws\Crypto\MaterialsProvider $provider, \NF_FU_VENDOR\Aws\Crypto\MetadataEnvelope $envelope)
    {
        ...
        if (!\NF_FU_VENDOR\Aws\Crypto\MaterialsProvider::isSupportedKeySize($cipherOptions['KeySize'])) {
        ...
        $envelope[\NF_FU_VENDOR\Aws\Crypto\MetadataEnvelope::CONTENT_KEY_V2_HEADER] = $provider->encryptCek($cek, $materialsDescription);
        unset($cek);
        $envelope[\NF_FU_VENDOR\Aws\Crypto\MetadataEnvelope::IV_HEADER] = \base64_encode($cipherOptions['Iv']);
        $envelope[\NF_FU_VENDOR\Aws\Crypto\MetadataEnvelope::KEY_WRAP_ALGORITHM_HEADER] = $provider->getWrapAlgorithmName();
        $envelope[\NF_FU_VENDOR\Aws\Crypto\MetadataEnvelope::CONTENT_CRYPTO_SCHEME_HEADER] = $aesName;
        $envelope[\NF_FU_VENDOR\Aws\Crypto\MetadataEnvelope::UNENCRYPTED_CONTENT_LENGTH_HEADER] = \strlen($plaintext);
        $envelope[\NF_FU_VENDOR\Aws\Crypto\MetadataEnvelope::UNENCRYPTED_CONTENT_MD5_HEADER] = \base64_encode(\md5($plaintext));
        $envelope[\NF_FU_VENDOR\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER] = \json_encode($materialsDescription);
        if (!empty($cipherOptions['Tag'])) {
            $envelope[\NF_FU_VENDOR\Aws\Crypto\MetadataEnvelope::CRYPTO_TAG_LENGTH_HEADER] = \strlen($cipherOptions['Tag']) * 8;
        }
     ...
     * @internal
     */
    protected function getEncryptingStream(\NF_FU_VENDOR\GuzzleHttp\Psr7\Stream $plaintext, $cek, &$cipherOptions)
    {
        switch ($cipherOptions['Cipher']) {
            case 'gcm':
                $cipherOptions['TagLength'] = 16;
                $cipherTextStream = new \NF_FU_VENDOR\Aws\Crypto\AesGcmEncryptingStream($plaintext, $cek, $cipherOptions['Iv'], $cipherOptions['Aad'] = isset($cipherOptions['Aad']) ? $cipherOptions['Aad'] : null, $cipherOptions['TagLength'], $cipherOptions['KeySize']);
                $appendStream = new \NF_FU_VENDOR\GuzzleHttp\Psr7\AppendStream([$cipherTextStream->createStream()]);
                $cipherOptions['Tag'] = $cipherTextStream->getTag();
                $appendStream->addStream(\NF_FU_VENDOR\GuzzleHttp\Psr7\stream_for($cipherOptions['Tag']));
            ...
            default:
                ...
                $cipherTextStream = new \NF_FU_VENDOR\Aws\Crypto\AesEncryptingStream($plaintext, $cek, $cipherMethod);
