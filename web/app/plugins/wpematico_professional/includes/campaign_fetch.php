...
if(!class_exists('WPeMaticoPro_Campaign_Fetch')) :
	...
	class WPeMaticoPro_Campaign_Fetch {
		...
		public static function fetching($campaign) {
			...
			if((isset($campaign['campaign_flip_paragraphs']) && $campaign['campaign_flip_paragraphs'])) {
				add_filter('wpematico_item_parsers', array(__CLASS__, 'campaignfetch_flip_paragraphs'), 900, 4);
...
*/
		...
		public static function assign_taxonomies($args, $campaign) {
			...
			if(isset($word2tax_options['word']) && (!empty($word2tax_options['word'][0]) ) && (!empty($word2tax_options['tax'][0]) ) && (!empty($word2tax_options['term'][0]) )
			) {
