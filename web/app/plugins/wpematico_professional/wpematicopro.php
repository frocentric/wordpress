...
if(!defined( 'WPEMATICOPRO_VERSION' ) ) {
	define( 'WPEMATICOPRO_VERSION', '2.4' );
} 
...
//			$cfg = get_option( self :: OPTION_KEY);
		...
		static function last_html_tag( $post, $cfgbasic ) { // part of basic template metabox
			...
			<div>
				<p id="w1">
					<b><?php _e('Last HTML tag to remove', 'wpematico' ); ?></b><span class="dashicons dashicons-warning help_tip" title="<?php echo $helptip['campaign_lastag_tag']; ?>"></span><br />
					<label for="campaign_lastag_tag"><?php _e('HTML tag:', 'wpematico' ); ?></label>
					<b><</b><input type="text" size="6" class="small-text" id="campaign_lastag_tag" name="campaign_lastag_tag" value="<?php echo stripslashes($campaign_lastag['tag']); ?>" /><b>></b> <?php _e('(ex: div, p, span, etc.)', 'wpematico' ); ?>
			...
			<div id="cfield_edit" class="inlinetext">		
				...
				<div class="<?php if(($i % 2) == 0) echo 'bw'; else echo 'lightblue'; ?> <?php if($i==count($campaign_cfields['name'])) echo 'hide'; ?>">
					<div class="clear pDiv jobtype-select rowflex" id="nuevocfield">
						...
						<div class="rowactions">
							<span class="" id="w2cactions">
								<label title="<?php _e('Delete this item', 'wpematico' ); ?>" onclick=" jQuery(this).parent().parent().parent().children('#cf1').children('#campaign_cf_name').val(''); jQuery(this).parent().parent().parent().fadeOut();" class="bicon delete left"></label>
							</span>
						</div>
					</div>
				</div>
