...
class Post_Grid_Widget extends \Elementor\Widget_Base {
	...
	public function generate_blog_post_classes( $classes ) {
		...
		if ( $wp_query->current_post === 0 && $paged === 1 && $settings['featured_column'] ) {
			$this->set_featured_column_classes( $classes );
		} else {
	...
	/**
	 * Sets classes for featured blog post
	 *
	 * @param string[] $classes  Array of CSS classes.
	 */
	protected function set_featured_column_classes( $classes ) {
		switch ( generate_blog_get_columns() ) {
			case 50:
				$classes[] = 'grid-100';
				break;
			case 33:
  ...
  // phpcs:ignore
	...
	protected function render() {
		...
		$the_query = $query_type === 'related' && is_singular( 'post' ) ? $this->get_related_posts( get_the_ID(), $page_size ) : new WP_Query(
			array(
